...
export function buildGroupsFromVariants({
  ...
  const colorAttr = product.productAttributes?.find((pa) => pa.attribute?.key === 'color');
  ...
  if (colorMap.size > 0) {
    groups.set(
      ...
      Array.from(colorMap.entries()).map(([value, variants]) => {
        ...
        const colorOpt = firstVariant?.options?.find(
          (o) =>
            (o.key === 'color' || o.attribute === 'color') &&
            o.value?.toLowerCase().trim() === value
        );
        const valueId = colorOpt?.valueId;
        const meta = colorAttr?.attribute?.values
          ? findAttributeValue(colorAttr.attribute.values, valueId, value, value)
          : null;
        ...
        return {
          ...
          colors: meta?.colors ?? (colorOpt?.colors as string[] | string | null | undefined) ?? null,
        };
      })
    );
  }
