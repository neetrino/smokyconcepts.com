...
export function findAttributeValue(
  ...
  value: string,
  label: string
): { imageUrl?: string | null; colors?: string[] | null; label?: string | null } | null {
  if (!attributeValues) return null;

  let attrValue: { imageUrl?: string | null; colors?: string[] | null; label?: string | null } | null = null;

  if (valueId) {
    // First try by valueId (most reliable)
    const found = (attributeValues as Array<{
      id?: string;
      label?: string;
      value?: string;
    ...
    if (found) {
      attrValue = {
        ...
        label: found.label ?? found.value ?? null,
      };
    }
  }
  ...
  if (!attrValue) {
    ...
    const found = (attributeValues as Array<{
      ...
      imageUrl?: string | null;
      colors?: string[] | null;
    }>).find((v) => v.value?.toLowerCase() === value?.toLowerCase() || v.value === value);
    if (found) {
      attrValue = {
        imageUrl: found.imageUrl ?? null,
        colors: found.colors ?? null,
  ...
  if (!attrValue) {
    ...
    const found = (attributeValues as Array<{
      ...
      imageUrl?: string | null;
      colors?: string[] | null;
    }>).find((v) => v.label?.toLowerCase() === label?.toLowerCase() || v.label === label);
    if (found) {
      attrValue = {
        imageUrl: found.imageUrl ?? null,
        colors: found.colors ?? null,
