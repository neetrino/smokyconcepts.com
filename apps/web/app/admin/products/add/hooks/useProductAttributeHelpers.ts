...
export function useProductAttributeHelpers({ attributes }: UseProductAttributeHelpersProps) {
  const colorAttribute = useMemo(() => {
    ...
    if (!colorAttr) {
      console.log('⚠️ [ADMIN] Color attribute not found. Available attributes:', attributes.map(a => ({ key: a.key, title: a.title })));
    } else {
  ...
  const sizeAttribute = useMemo(() => {
    ...
    if (!sizeAttr) {
      console.log('⚠️ [ADMIN] Size attribute not found. Available attributes:', attributes.map(a => ({ key: a.key, title: a.title })));
    } else {
