    ...
    <Card className="p-4 md:p-6">
      ...
      <div className="space-y-3">
        <div className="flex justify-between text-sm text-gray-700">
          ...
          <span>{formatCurrency(orderDetails.totals.subtotal, (orderDetails.totals.currency || 'USD') as CurrencyCode, 'USD')}</span>
        </div>
        {orderDetails.totals.discount > 0 && (
          <div className="flex justify-between text-sm text-gray-700">
            <span>{t('orders.orderSummary.discount')}</span>
            <span>-{formatCurrency(orderDetails.totals.discount, (orderDetails.totals.currency || 'USD') as CurrencyCode, 'USD')}</span>
          </div>
        )}
        <div className="flex justify-between text-sm text-gray-700">
          ...
          <span>
            {orderDetails.shippingMethod === 'pickup'
              ...
              : formatCurrency(orderDetails.totals.shipping, orderDetails.totals.currency || 'USD', 'USD') + (orderDetails.shippingAddress?.city ? ` (${orderDetails.shippingAddress.city})` : '')}
          </span>
        </div>
        <div className="flex justify-between text-sm text-gray-700">
          <span>{t('orders.orderSummary.tax')}</span>
          <span>{formatCurrency(orderDetails.totals.tax, (orderDetails.totals.currency || 'USD') as CurrencyCode, 'USD')}</span>
        </div>
