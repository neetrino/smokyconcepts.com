...
import { HomeHeroSlideHeader } from './HomeHeroSlideHeader';
...
function slideCardClassName(isDragging: boolean, isDropTarget: boolean): string {
  if (isDragging) {
    return 'rounded-xl border border-[#dcc090] bg-[#dcc090]/20 opacity-60 transition-all duration-200';
  }
  if (isDropTarget) {
    return 'rounded-xl border border-[#122a26] bg-[#dcc090]/25 transition-all duration-200';
  }
  return 'rounded-xl border border-[#dcc090]/25 bg-white/95 transition-all duration-200 hover:border-[#dcc090]/50 hover:shadow-[0_4px_16px_rgba(18,42,38,0.06)]';
}
...
export function HomeHeroSlideEditor({
  ...
  return (
    <div
      className={slideCardClassName(isDragging, isDropTarget)}
      onDragOver={(event) => {
