...
import { readRememberedPaymentReturnOrderNumber } from '@/lib/payments/remember-payment-return-order-number';
...
export default function CheckoutPaymentFailedPage() {
  ...
  const queryOrderNumber = resolvePaymentReturnOrderNumber(searchParams);
  const [rememberedOrderNumber, setRememberedOrderNumber] = useState<string | null>(null);
  const orderNumber = queryOrderNumber ?? rememberedOrderNumber;
  ...
  useEffect(() => {
    ...
    if (!queryOrderNumber) {
      setRememberedOrderNumber(readRememberedPaymentReturnOrderNumber());
    }
  }, [queryOrderNumber]);
    ...
    <div className="mx-auto max-w-3xl px-4 py-16 sm:px-6 lg:px-8">
      <Card className="p-8 text-center sm:p-10">
        ...
        <p className="mt-3 text-base text-gray-600">
          We could not confirm your payment. Please try again or choose another payment method.
        </p>
