import { CATALOG_MOBILE_STRIP_FLEX_SNAP_CLASS_NAME } from '../../app/products/components/catalogProductCardMobilePresentation';
...
/** Mobile: tighter strip padding (scroll-only; no dot row below `sm`). */
export const UPCOMING_MOBILE_SCROLL_TOP_PADDING_CLASS = 'max-sm:pt-[5.5rem]';
export const UPCOMING_MOBILE_SCROLL_BOTTOM_PADDING_CLASS = 'max-sm:pb-6';
...
export const UPCOMING_PRODUCT_STRIP_FLEX_CLASS_NAME =
  `flex min-w-max items-stretch max-sm:gap-4 max-sm:pr-5 gap-4 sm:gap-7 lg:gap-10 lg:pr-[7.5rem] ${CATALOG_MOBILE_STRIP_FLEX_SNAP_CLASS_NAME} ${UPCOMING_DESKTOP_STRIP_HERO_HORIZONTAL_BLEED_CLASS_NAME}`;
...
export const UPCOMING_SCROLL_CONTAINER_CLASS_NAME =
  `scrollbar-hide max-sm:mt-1 mt-3 overflow-x-auto overflow-y-visible overscroll-x-contain pb-10 ${UPCOMING_MOBILE_SCROLL_BOTTOM_PADDING_CLASS} pt-[5.5rem] ${UPCOMING_MOBILE_SCROLL_TOP_PADDING_CLASS} ${UPCOMING_MOBILE_SCROLL_BLEED_CLASS} sm:mt-6 sm:mx-0 sm:px-0 sm:pb-12 sm:pt-[6rem] lg:pb-14 lg:pt-[5.75rem]`;
