...
export const DISPLAY_VARIANT_ATTRIBUTE_KEY = "__display_variant__";
...
export function isDisplayVariant(variant: VariantLike | null | undefined): boolean {
  if (!variant) {
    return false;
  }
  ...
  return attributes.some((item) => {
    ...
    return entry.attributeKey === DISPLAY_VARIANT_ATTRIBUTE_KEY;
  });
}

export function buildSizeCatalogAttributes(sizeCatalogSelection?: {
  categoryId?: string | null;
  categoryTitle?: string | null;
}): Array<{ attributeKey: string; value: string }> {
  const attributes: Array<{ attributeKey: string; value: string }> = [];
...
export function buildDisplayVariantAttributes(sizeCatalogSelection?: {
  categoryId?: string | null;
  categoryTitle?: string | null;
}): Array<{ attributeKey: string; value: string }> {
  return [
    { attributeKey: DISPLAY_VARIANT_ATTRIBUTE_KEY, value: "true" },
    ...buildSizeCatalogAttributes(sizeCatalogSelection),
  ];
}
